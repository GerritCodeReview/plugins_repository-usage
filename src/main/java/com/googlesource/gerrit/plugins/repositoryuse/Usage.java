...
public class Usage {
  private static final Logger log =
      LoggerFactory.getLogger(Usage.class);
  ...
  public Usage(String project, String branch, String destination, String ref, String info) {
    init(project, branch, destination, ref, info, new Date());
  }

  public Usage(String project, String branch, String destination, String ref, String info, Date date) {
  ...
  public void save() {
    ...
    log.info(String.format("Saving Usage: %s, %s, %s, %s", project, branch, destination, ref));
  }

  public void delete() {
    table.delete(this);
    log.info(String.format("Deleting Usage: %s, %s, %s", project, branch, destination));
  }
  ...
  static class Table {
    ...
    private void createTable() {
      ...
      query.append(String.format("PRIMARY KEY (%s, %s, %s))", PROJECT, BRANCH, DESTINATION));
      try {
    ...
    public void insertOrUpdate(Usage u) {
      if (fetchByProject(u.getProject(), u.getBranch(), u.getDestination()).isEmpty()) {
        String query = "INSERT INTO " + TABLE_NAME + "(" + PROJECT + ", " + BRANCH + ", " + DESTINATION + ", " + REF + ", " + INFO + ", " + DATE + ") VALUES (?, ?, ?, ?, ?, " + sql.getDateFormat() + ")";
        try {
          sql.execute(query, u.getProject(), u.getBranch(), u.getDestination(), u.getRef(), u.getInfo(), sql.getDateAsString(u.getLastUpdated()));
        } catch (SQLException e) {
          log.error("Unable to insert usage", e);
        }
      } else {
        String query = "UPDATE " + TABLE_NAME + " SET " + REF + "=?, " + INFO + "=?, " + DATE + "=" + sql.getDateFormat() + " WHERE " + PROJECT + "=? AND " + BRANCH + "=? AND " + DESTINATION + "=?";
        try {
          sql.execute(query, u.getRef(), u.getInfo(), sql.getDateAsString(u.getLastUpdated()), u.getProject(), u.getBranch(), u.getDestination());
        } catch (SQLException e) {
    ...
    public void delete(Usage u) {
      String query = "DELETE FROM " + TABLE_NAME + " WHERE " + PROJECT + "=? AND " + BRANCH + "=? AND " + DESTINATION + "=?";
      try {
    ...
    public void deleteByBranch(String project, String branch) {
      String query = "DELETE FROM " + TABLE_NAME + " WHERE " + PROJECT + "=? AND " + BRANCH + "=?";
      try {
    ...
    public List<Usage> fetchByProject(String project) {
      String query = "SELECT " + PROJECT + ", " + BRANCH + ", " + DESTINATION + ", " + REF + ", " + INFO + ", "
    ...
    public List<Usage> fetchByProject(String project, String branch) {
      String query = "SELECT " + PROJECT + ", " + BRANCH + ", " + DESTINATION + ", " + REF + ", " + INFO + ", "
          + DATE + " FROM " + TABLE_NAME + " WHERE " + PROJECT + "=? AND " + BRANCH + "=?";
      try {
    ...
    public List<Usage> fetchByProject(String project, String branch, String destination) {
      String query = "SELECT " + PROJECT + ", " + BRANCH + ", " + DESTINATION + ", " + REF + ", " + INFO + ", "
          + DATE + " FROM " + TABLE_NAME + " WHERE " + PROJECT + "=? AND " + BRANCH + "=? AND " + DESTINATION + "=?";
      try {
    ...
    public List<Usage> fetchByDependency(String dependency) {
      String query = "SELECT " + PROJECT + ", " + BRANCH + ", " + DESTINATION + ", " + REF + ", " + INFO + ", "
          + DATE + " FROM " + TABLE_NAME + " WHERE " + DESTINATION + "=?";
      try {
    ...
    private List<Usage> loadUsage(List<Map<String, String>> rows) {
      ...
      for (Map<String, String> row : rows) {
        Usage tmp =
            new Usage(row.get(PROJECT), row.get(BRANCH), row.get(DESTINATION), row.get(REF), row.get(INFO), sql.getStringAsDate(row.get(DATE)));
        result.add(tmp);
      }
      return result;
    }
