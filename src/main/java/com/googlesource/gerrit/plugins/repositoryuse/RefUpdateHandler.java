...
public class RefUpdateHandler implements GitReferenceUpdatedListener {
  ...
  @Inject
  RefUpdateHandler(ProjectControl.GenericFactory p,
      CurrentUser user,
  ...
  private void parseManifests(Event event, ProjectResource project) throws RepositoryNotFoundException, IOException {
    if (event.isDelete()) {
      return;
    }
   ...
   */
  private boolean isSubmoduleUpdate(Event event, ProjectResource project)
  ...
  private Map<String,String> getSubmodules(Event event, ProjectResource project)
      throws RepositoryNotFoundException, IOException {
