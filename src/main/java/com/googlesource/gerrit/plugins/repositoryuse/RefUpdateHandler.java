...
public class RefUpdateHandler implements GitReferenceUpdatedListener {
  ...
  @Inject
  RefUpdateHandler(ProjectControl.GenericFactory p, CurrentUser user,
    ...
    if (event.isDelete() && event.getRefName().startsWith(Constants.R_HEADS)
      ...
      try {
        ...
        if (Config.refreshAllSubmodules() || event.isCreate()
            || isSubmoduleUpdate(event, project)) {
  ...
  private void parseManifests(Event event, ProjectResource project)
  ...
  private Map<String, String> getSubmodules(Event event,
      ProjectResource project) throws RepositoryNotFoundException, IOException {
